...
public class SQLAgentPlugin   // Defines a plugin for SQL-related tasks integrated with AI
{
    ...
    public SQLAgentPlugin(IDbConnection db, Kernel kernel, IMemoryCache cache)
    {
    ...
    public async Task<string> GetSchemaAsync()
    {
        ...
        // Query all user-defined tables from SQLite, excluding internal system tables
        var tables = await _db.QueryAsync<string>(
            "SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'");
       
        // Dictionary to store table names with their columns
        var schema = new Dictionary<string, List<string>>();

        // For each table, fetch its column names
        foreach (var table in tables)
        {
            var cols = await _db.QueryAsync<dynamic>($"PRAGMA table_info({table})");
            schema[table] = cols.Select(c => (string)c.name).ToList(); // Add column names to schema
        }

        // Serialize schema dictionary to JSON and cache it
        _cachedSchema = JsonSerializer.Serialize(schema);
        return _cachedSchema; // Return schema JSON
    }
